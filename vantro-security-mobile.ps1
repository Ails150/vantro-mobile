Write-Host "=== VANTRO MOBILE - GPS ACKNOWLEDGMENT SCREEN ===" -ForegroundColor Cyan
Write-Host ""

# ─── 1. CREATE GPS ACKNOWLEDGMENT SCREEN ─────────────────────────────
$ackScreen = @'
...
New-Item -ItemType Directory -Force -Path "C:\vantro-mobile\app\(installer)" | Out-Null
[System.IO.File]::WriteAllText("C:\vantro-mobile\app\(installer)\gps-consent.tsx", $ackScreen, [System.Text.UTF8Encoding]::new($false))
Write-Host "1/2 GPS acknowledgment screen created" -ForegroundColor Green

# ─── 2. UPDATE AUTH CONTEXT — redirect to consent if not acknowledged ─
$authContextPath = "C:\vantro-mobile\context\AuthContext.tsx"
if (Test-Path $authContextPath) {
  $authCtx = [System.IO.File]::ReadAllText($authContextPath, [System.Text.UTF8Encoding]::new($false))

  # Add gpsAcknowledged to the auth state if not present
  if ($authCtx -notmatch "gpsAcknowledged") {
    # Add to user state
    $authCtx = $authCtx -replace 'setUser\(\{ name: data\.name, role: data\.role', 'setUser({ name: data.name, role: data.role, gpsAcknowledged: data.gpsAcknowledged'
    $authCtx = $authCtx -replace 'setUser\(\{ name, role', 'setUser({ name, role, gpsAcknowledged: true'

    [System.IO.File]::WriteAllText($authContextPath, $authCtx, [System.Text.UTF8Encoding]::new($false))
    Write-Host "2/2 AuthContext updated with gpsAcknowledged flag" -ForegroundColor Green
  } else {
    Write-Host "2/2 AuthContext already has gpsAcknowledged - skipping" -ForegroundColor Yellow
  }
} else {
  Write-Host "2/2 AuthContext not found at expected path - you may need to update routing manually" -ForegroundColor Yellow
}

# ─── COMMIT ─────────────────────────────────────────────────────────
cd C:\vantro-mobile
git add .
git commit -m "Security: GPS tracking acknowledgment screen, privacy policy link"

Write-Host ""
Write-Host "=== MOBILE SECURITY UPDATE DONE ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "The GPS acknowledgment screen shows on first login if gpsAcknowledged is false." -ForegroundColor White
Write-Host "After acknowledging, the installer proceeds to jobs as normal." -ForegroundColor White
Write-Host ""
Write-Host "To test: build preview APK and login with a test installer" -ForegroundColor Yellow
